...
exports.getAssessmentComponentOptions = async (req, res) => {
  try {
    ...
    res.json({
      ...
      componenttypes: Array.from(allowedComponentTypes),
...
exports.getAssessmentComponentAiOptions = async (req, res) => {
  try {
    const colid = toNumber(req.query.colid);
    if (colid === undefined) return res.status(400).json({ success: false, message: "colid is required" });
...
exports.validateProgramAssessmentComponentWithAi = async (req, res) => {
  try {
    ...
    const [courses, assessments] = await Promise.all([
      ...
      AssessmentComponent.find(assessmentQuery).sort({ regulation: 1, semester: 1, coursecode: 1, assessmentgroup: 1, scoretype: 1, componenttype: 1 }).lean()
    ]);
    ...
    const prompt = `
You are validating assessment component schemes for every course in one academic program.
...
exports.createAssessmentComponent = async (req, res) => {
  try {
    ...
    const data = await AssessmentComponent.create(payload);
...
exports.getAssessmentComponents = async (req, res) => {
  try {
    const query = buildQuery(req.query);
    if (query.colid === undefined) return res.status(400).json({ success: false, message: "colid is required" });
    const data = await AssessmentComponent.find(query).sort({ academicyear: 1, regulation: 1, program: 1, type: 1, course: 1, assessmentgroup: 1, grouptype: 1, scoretype: 1, componenttype: 1, assessmentcomponent: 1 });
...
exports.updateAssessmentComponent = async (req, res) => {
  try {
    ...
    const error = validatePayload(payload);
    if (error) return res.status(400).json({ success: false, message: error });
    const data = await AssessmentComponent.findByIdAndUpdate(req.body.id, payload, { new: true, runValidators: true });
...
exports.deleteAssessmentComponent = async (req, res) => {
  try {
    const data = await AssessmentComponent.findByIdAndDelete(req.body.id);
    if (!data) return res.status(404).json({ success: false, message: "Record not found" });
...
exports.bulkCreateAssessmentComponents = async (req, res) => {
  try {
    ...
    if (valid.length) await AssessmentComponent.insertMany(valid, { ordered: false });
