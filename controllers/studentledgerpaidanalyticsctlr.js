...
exports.getStudentLedgerPaidAnalytics = async (req, res) => {
  try {
    ...
    const data = await Ledgerstud.find(query)
      .select("academicyear regulation programcode feegroup feeitem feebook cashbook status major minor semester feecategory paiddate amount concession paid balance colid")
      .sort({ paiddate: -1, academicyear: -1, programcode: 1, feegroup: 1, feeitem: 1 })
      .limit(5000)
      .lean();
